...
function clipboard(api, code = 'npm run dev\n') {
  ...
  return createCopyCode({
    querySelector(selector) {
      return {textContent: selector === '[data-code-label]' ? 'Terminal one' : code};
    },
  });
}
...
test('clipboard absence and missing code never announce success', async () => {
  for (const create of [
    ...
    await assert.doesNotReject(component.copy());
    assert.equal(component.status, 'Copy unavailable. Select the code to copy it.');
    component.destroy();
  }
});
...
test('desktop reflow moves focus out of the hidden mobile disclosure', () => {
  const {component, menu, trigger, desktop, document, window, sections} = navigation();
