...
for (const [route, html] of pages) {
  ...
  const llms = links.find(
    (tag) =>
      attribute(tag, 'rel') === 'alternate' && attribute(tag, 'type') === 'text/plain',
  );
  assert(llms, `${route}: AI-readable documentation link`);
  assert.equal(new URL(attribute(llms, 'href'), pageURL).pathname, '/llms.txt');
  assert.equal(
  ...
  if (route === '/') {
    ...
    assert.match(
      html,
      /Creates an Easel\/Vite starter with your selected integrations and storefront\s+templates/,
    );
    assert.match(
      html,
    ...
    assert.match(html, /browsers defer modules automatically/);
    assert.match(html, /refresh\.signal/);
    assert.match(html, /refresh\.delay/);
    assert.match(html, /milliseconds/);
    assert.match(html, /Choosing Easel\./);
    assert.match(html, /A good fit/);
    assert.match(html, /Choose another approach when/);
    assert.match(html, /Hydrogen or other custom storefront/);
...
for (const text of [
  ...
  'Shopify CLI remains responsible',
  'https://easel.billynoyes.co.uk/docs/',
]) {
  assert(llms.includes(text), `llms.txt includes ${text}`);
}
