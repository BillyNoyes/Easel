...
import {createEaselManifest} from './manifest.js';
...
export function finalizeProductionBuild(
  ...
  try {
    ...
    const manifest = createEaselManifest(
      viteManifest,
      options.bundles,
...
function readViteManifest(stagingPath: string): Manifest {
  ...
  if (!existsSync(path)) {
    throw new Error(`[easel] Vite did not write its build manifest: ${path}`);
  }
  try {
    return JSON.parse(readFileSync(path, 'utf8')) as Manifest;
  } catch (error) {
    throw new Error(`[easel] Vite wrote an invalid build manifest: ${path}`, {
      cause: error,
    });
  }
}
...
function removeStagingDirectory(stagingPath: string, config: ResolvedConfig): void {
  try {
    ...
    config.logger.warn(
      `[easel] could not remove staging directory ${stagingPath}: ${errorMessage(error)}`,
    );
  }
}
...
function listEmittedFiles(stagingPath: string): string[] {
  ...
  for (const entry of readdirSync(stagingPath, {withFileTypes: true})) {
    ...
    if (!entry.isFile()) {
      throw new Error(`[easel] unexpected output in staging directory: ${entry.name}`);
    }
